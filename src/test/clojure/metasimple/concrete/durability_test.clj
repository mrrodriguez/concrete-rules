(ns metasimple.concrete.durability-test
  (:require
    [clojure.java.io :as jio]
    [clojure.test :refer :all]
    [metasimple.concrete.durability-rules :as dr]
    ...
    [metasimple.concrete.rules.durability.fressian :as df]
  ...
  (when (is (= expected-fact fact)
    ...
    (or (identical? expected-fact fact)
      (and (is (= (coll? expected-fact)
        ...
        ;; This is only using the rules from metasimple.concrete.durability-rules.
...
(defn durability-test
  ...
  [serde-type]
  (let [s (mk-session 'metasimple.concrete.durability-rules)
...
(deftest test-assemble-restored-session-opts
  (let [orig (mk-session 'metasimple.concrete.durability-rules)
...
(deftest test-compilation-ctx
  ...
  (let [rule (dsl/parse-rule [[Long (== this test-compilation-ctx-var)]]
    ...
    (ns-unmap 'metasimple.concrete.durability-test 'test-compilation-ctx-var)
    (try
      ...
      (catch Exception e
        ...
        (is (some #(and (not (contains? % :condition))
                     ;; Validate that the exception is related to the Concrete Rules compilation failure (and so the check above is valid).
                     (:expr %))
    ...
    (try
      ...
      (catch Exception e
        ...
        (is (some #(= (select-keys (:condition %) [:type :constraints])
                     {:type  Long
                      :constraints ['(== this metasimple.concrete.durability-test/test-compilation-ctx-var)]})
              (tu/get-all-ex-data e)))))))
