(ns metasimple.concrete.rules-test
  (:require
   [clojure.edn :as edn]
   [clojure.test :refer :all]
   [metasimple.concrete.order-ruleset :as order-rules]
   ...
   [metasimple.concrete.rules.dsl :as dsl]
   [metasimple.concrete.rules.engine :as eng]
   [metasimple.concrete.rules.rules-data-test :as rules-data]
   [metasimple.concrete.rules.testfacts :as tf]
   [metasimple.concrete.sample-ruleset :as sample]
   [metasimple.concrete.sample-ruleset-seq :as srs]
   [metasimple.concrete.tools.testing-utils :as tu :refer [assert-ex-data]]
   [metasimple.concrete.tools.tracing :as t]
   [schema.test])
  (:import
   (java.util
    ArrayList
    ...
    List)
   (metasimple.concrete.rules.engine
    ISession)
...
(deftest test-accum-result-in-negation
  (let [all-temps-are-max (dsl/parse-query
    ...
    (is (empty?
         (-> session
             (insert (tf/->Temperature 50 "MCI"))
             (insert (tf/->Temperature 40 "MCI"))
             fire-rules
...
(deftest test-retraction-of-join
  (let [same-wind-and-temp (dsl/parse-query [] [[Temperature (= ?t temperature)]
        ...
        session (-> (mk-session [same-wind-and-temp])
                    (insert (tf/->Temperature 10 "MCI"))
                    (insert (tf/->WindSpeed 10 "MCI"))
                    fire-rules)

        retracted-session (-> session
                              (retract (tf/->Temperature 10 "MCI"))
                              fire-rules)]
...
(deftest test-negation-of-changing-result-from-accumulator-in-fire-rules
  (let [min-temp-rule (dsl/parse-rule [[?c <- (acc/min :temperature :returns-fact true) :from [ColdAndWindy]]]
        ...
        first-cold (dsl/parse-rule [[First]]
                                   (insert! (tf/->ColdAndWindy 20 20))
                                   {:salience -1})

        second-cold (dsl/parse-rule [[Second]]
                                    (insert! (tf/->ColdAndWindy 10 10))
                                    {:salience -2})
...
(deftest test-simple-disjunction
  (let [or-query (dsl/parse-query [] [[:or [Temperature (< temperature 20) (= ?t temperature)]
        ...
        cold-session (-> session
                         (insert (tf/->Temperature 15 "MCI"))
                         fire-rules)
        windy-session (-> session
                          (insert (tf/->WindSpeed 50 "MCI"))
                          fire-rules)]
...
(deftest test-multi-conditions-with-nested-conjunction-inside-disjunction
  ...
  (let [find-conditions (dsl/parse-query []
        ...
        temp (tf/->Temperature 10 "MCI")
        cold (tf/->Cold 10)
        wind-speed (tf/->WindSpeed 50 "MCI")
        cold-and-windy (tf/->ColdAndWindy 10 80)
...
(deftest test-simple-defrule
  (let [session (-> (mk-session [test-rule])
    ...
    (is (has-fact? @simple-defrule-side-effect (tf/->Temperature 10 "MCI")))
    (is (has-fact? @other-defrule-side-effect (tf/->Temperature 10 "MCI")))))
...
(deftest test-defquery
  (let [session (-> (mk-session [cold-query hot-query])
                    (insert (tf/->Temperature 15 "MCI"))
                    (insert (tf/->Temperature 20 "MCI")) ; Test multiple items in result.
                    (insert (tf/->Temperature 10 "ORD"))
                    (insert (tf/->Temperature 35 "BOS"))
                    (insert (tf/->Temperature 80 "BOS"))
                    fire-rules)]
...
(deftest test-rules-from-ns
  ...
  (doseq [rules-ns ['metasimple.concrete.sample-ruleset
                    'metasimple.concrete.sample-ruleset-seq]]
    (is (= #{{:?loc "MCI"} {:?loc "BOS"}}
           (set (-> (mk-session rules-ns)
                    (insert (tf/->Temperature 15 "MCI"))
                    (insert (tf/->Temperature 22 "BOS"))
                    (insert (tf/->Temperature 50 "SFO"))
                    fire-rules
    ...
    (let [session (-> (mk-session rules-ns)
                      (insert (tf/->Temperature 15 "MCI"))
                      (insert (tf/->WindSpeed 45 "MCI"))
                      (fire-rules))]

      (is (= #{{:?fact (tf/->ColdAndWindy 15 45)}}
             (set
...
(deftest test-rules-from-multi-namespaces
  (doseq [sample-ruleset-ns ['metasimple.concrete.sample-ruleset
                             'metasimple.concrete.sample-ruleset-seq]]
    (let [session (-> (mk-session sample-ruleset-ns 'metasimple.concrete.other-ruleset)
                      (insert (tf/->Temperature 15 "MCI"))
                      (insert (tf/->Temperature 10 "BOS"))
                      (insert (tf/->Temperature 50 "SFO"))
                      (insert (tf/->Temperature -10 "CHI"))
                      fire-rules)]
...
(deftest test-transitive-rule
  (doseq [sample-ruleset-ns ['metasimple.concrete.sample-ruleset
                             'metasimple.concrete.sample-ruleset-seq]]
    (is (= #{{:?fact (tf/->LousyWeather)}}
           (set (-> (mk-session sample-ruleset-ns 'metasimple.concrete.other-ruleset)
                    (insert (tf/->Temperature 15 "MCI"))
                    (insert (tf/->WindSpeed 45 "MCI"))
                    (fire-rules)
...
(deftest test-mark-as-fired
  (let [rule-output (atom nil)
    ...
    (is (has-fact? @rule-output (tf/->Temperature 10 "MCI")))
...
(deftest test-chained-inference
  (let [item-query (dsl/parse-query [] [[?item <- Fourth]])
        ...
        session (-> (mk-session [(dsl/parse-rule [[Third]] (insert! (tf/->Fourth)))
                                 (dsl/parse-rule [[First]] (insert! (tf/->Second)))
                                 (dsl/parse-rule [[Second]] (insert! (tf/->Third)))
                                 item-query])
                    (insert (tf/->First))
                    (fire-rules))]
...
(deftest test-multi-insert-retract
  ...
  (is (= #{{:?loc "MCI"} {:?loc "BOS"}}
         (set (-> (mk-session 'metasimple.concrete.sample-ruleset)
                  (insert (tf/->Temperature 15 "MCI"))
                  (insert (tf/->Temperature 22 "BOS"))

               ;; Insert a duplicate and then retract it.
                  (insert (tf/->Temperature 22 "BOS"))
                  (retract (tf/->Temperature 22 "BOS"))
                  (fire-rules)
                  (query sample/freezing-locations)))))
  ...
  (is (= #{}
         (set (-> (mk-session 'metasimple.concrete.sample-ruleset)
                  (insert (tf/->Temperature 15 "MCI"))
                  (insert (tf/->Temperature 22 "BOS"))
                  (retract (tf/->Temperature 22 "BOS") (tf/->Temperature 15 "MCI"))
                  (fire-rules)
                  (query sample/freezing-locations)))))

  (let [session (-> (mk-session 'metasimple.concrete.sample-ruleset)
                    (insert (tf/->Temperature 15 "MCI"))
                    (insert (tf/->WindSpeed 45 "MCI"))

                  ;; Insert a duplicate and then retract it.
                    (insert (tf/->WindSpeed 45 "MCI"))
                    (retract (tf/->WindSpeed 45 "MCI"))
                    (fire-rules))]
...
(defrule reduce-temp-no-loop
  ...
  (do
    (retract! ?t)
    (insert-unconditional! (tf/->Temperature (- ?v 1) "MCI"))))
...
(deftest test-identical-facts
  (let [ident-query (dsl/parse-query [] [[?t1 <- Temperature (= ?loc location)]
        ...
        temp (tf/->Temperature 15 "MCI")
        temp2 (tf/->Temperature 15 "MCI")
    ...
    (is (= [{:?t1 #metasimple.concrete.rules.testfacts.Temperature{:temperature 15, :location "MCI"}
             :?t2 #metasimple.concrete.rules.testfacts.Temperature{:temperature 15, :location "MCI"}
             :?loc  "MCI"}
            {:?t2 #metasimple.concrete.rules.testfacts.Temperature{:temperature 15, :location "MCI"}
             :?t1 #metasimple.concrete.rules.testfacts.Temperature{:temperature 15, :location "MCI"}
             :?loc "MCI"}]
...
;; sequence of maps matching the metasimple.concrete.rules.schema/Production schema.
(def external-rules "[{:name \"cold-query\", :params #{:?l}, :lhs [{:type metasimple.concrete.rules.testfacts.Temperature, :constraints [(< temperature 50) (= ?t temperature) (= ?l location)]}]}]")
...
(defsession my-session 'metasimple.concrete.sample-ruleset)
...
(deftest test-override-ancestors
  (let [special-ancestor-query (dsl/parse-query [] [[?result <- :my-ancestor]])
    ...
    (is (= #{{:?result (tf/->Temperature 15 "MCI")}
             {:?result (tf/->Temperature 10 "MCI")}
             {:?result (tf/->Temperature 80 "MCI")}}
...
(deftest test-accumulator-with-extracted-test
  (let [q1 (dsl/parse-query []
    ...
    (is (= [{:?res [(tf/->Temperature 9 "MCI")] :?t 9}]
           (-> (mk-session [q1])
               (insert (tf/->Temperature 9 "MCI"))
               (fire-rules)
               (query q1))))

    (is (= [{:?res [(tf/->Temperature 9 "MCI")]}]
           (-> (mk-session [q2])
               (insert (tf/->Temperature 9 "MCI"))
               (fire-rules)
               (query q2))))))
...
(deftest test-destructured-binding
  (let [rule-output-env (atom nil)
        rule {:name "metasimple.concrete.test-destructured-binding/test-destructured-binding"
...
(deftest test-destructured-test-env-binding
  (let [rule-output-env (atom nil)
        rule {:name "metasimple.concrete.test-destructured-binding/test-destructured-test-env-binding"
              :env {:rule-output rule-output-env} ; Rule environment so we can check its output.
              :lhs '[{:args [[e a v]]
                      :type :foo
...
(deftest test-explicit-rhs-map-can-use-ns-name-for-unqualified-symbols
  (let [;; The :rhs form only makes sense within the scope of another
        ;; namespace - i.e. metasimple.concrete.sample-ruleset-seq.
        ;; Demonstrating this can work, independently of the Concrete Rules DSL.
        r1 {:ns-name 'metasimple.concrete.sample-ruleset-seq
            :name "r1"
            :lhs '[{:type :test
                    :constraints []}]
            :rhs '(metasimple.concrete.rules/insert! ^{:type :result}
                   {:r :r1
                    :v all-rules})}
...
(deftest test-qualified-equals-for-fact-binding
  (let [get-accum-nodes #(->> %
                              ...
                              first
                              :children
                              (filter (partial instance? metasimple.concrete.rules.engine.AccumulateNode)))
...
(deftest test-non-list-seq-form-used-for-non-eq-unification
  (let [non-list-constraint (cons '= '(this (identity ?t)))
        ...
        q {:lhs [{:type metasimple.concrete.rules.testfacts.Temperature
                  :constraints []
                  :fact-binding :?t}
                 {:type metasimple.concrete.rules.testfacts.Temperature
                  :constraints [non-list-constraint]}]
           :params #{}}
        temp (tf/->Temperature 1 "MCI")
...
(deftest test-complex-nested-truth-maintenance-with-unconditional-insert
  (let [r (dsl/parse-rule [[Cold (= ?t temperature)]
                           [:not [:or [ColdAndWindy (= ?t temperature)]
                                  [Hot (= ?t temperature)]]]]
                          (insert-unconditional! (tf/->First)))

        q (dsl/parse-query [] [[First]])

        session->results (fn [session]
                           (-> session
                               (insert (tf/->Cold 10) (tf/->Hot 10))
                               (fire-rules)
                               (query q)))]
...
(def some-rules
  [(assoc (dsl/parse-query [] [[Temperature (< temperature 40) (= ?t temperature) (= ?l location)]])
          :name "cold")
...
(deftest load-rules-from-qualified-symbol
  ...
  (let [session (-> (mk-session 'metasimple.concrete.rules-test/cold-query)
                    (insert (tf/->Temperature 35 "BOS"))
                    (fire-rules))]
  ...
  (let [session (-> (mk-session 'metasimple.concrete.rules-test/some-rules)
                    (insert (tf/->Temperature 35 "BOS"))
                    (insert (tf/->Temperature 85 "MIA"))
                    (fire-rules))]
  ...
  (is (thrown? clojure.lang.ExceptionInfo
               (mk-session 'metasimple.concrete.rules-test/bogus-symbol))))
  ...
  (let [results (atom [])
        ...
        r4 (dsl/parse-rule [[?ts <- (acc/all :temperature) :from [Temperature]]]
                           (do (swap! results conj ?ts)
                               (insert! (tf/->TemperatureHistory ?ts)))
                           {:salience -1})
...
(deftest test-rule-order-respected
  (let [fire-order (atom [])
    ...
    (binding [order-rules/*rule-order-atom* fire-order]
      (-> (mk-session [rule-A] 'metasimple.concrete.order-ruleset :cache false)
          (insert (tf/->Cold 10))
          fire-rules)
      (is (= @fire-order [:A :C :D])
    ...
    (binding [order-rules/*rule-order-atom* fire-order]
      (-> (mk-session 'metasimple.concrete.order-ruleset [rule-A] :cache false)
          (insert (tf/->Cold 10))
          fire-rules)
      (is (= @fire-order [:C :D :A])
    ...
    (let [rule-C-line (-> #'order-rules/rule-C meta :line)
      ...
      (binding [order-rules/*rule-order-atom* fire-order]
        (-> (mk-session 'metasimple.concrete.order-ruleset :cache false)
            (insert (tf/->Cold 10))
            fire-rules))
...
(deftest test-rule-order-respected-by-batched-inserts
  (let [qholder (atom [])
    ...
    (is (= (frequencies [{:?c (tf/->Cold 10)}])
           res11
           res21))
    (is (= (frequencies [{:?h (tf/->Hot 10)}])
           res12
           res22))
...
(deftest test-extra-right-activations-with-disjunction-of-negations
  (let [no-join (dsl/parse-rule [[:or
    ...
    (doseq [[empty-session join-type] [[(mk-session [no-join q] :cache false) "simple hash join"]
      ...
      (is (= (-> empty-session
                 (insert (tf/->Hot 100) (tf/->Temperature -100 "ORD"))
                 fire-rules
                 (insert (tf/->Hot 120))
                 fire-rules
                 (query q))
...
(deftest test-alpha-batching-with-multiple-ancestors
  ...
  ;; access the facts passed to the condition without exposing Concrete Rules internals.
...
(deftest test-single-condition-queries-constraint-exception
  (let [query-template (dsl/parse-query [] [[Temperature
        ...
        temperature-fact (tf/->Temperature nil "MCI")]

    (assert-ex-data {:bindings nil
                     :fact temperature-fact
                     :conditions-and-rules
                     {[metasimple.concrete.rules.testfacts.Temperature '(= ?location location) '(> temperature 0)]
...
(deftest test-single-condition-multiple-rule-constraint-exception
  (let [rule-one (dsl/parse-rule [[WindSpeed
    ...
    (assert-ex-data {:bindings nil
                     :fact temperature-fact
                     :conditions-and-rules
                     {[metasimple.concrete.rules.testfacts.Temperature '(> temperature 0)]
...
(deftest test-single-condition-negation-rules-constraint-exception
  (let [rule-template (dsl/parse-rule [[:not [Temperature (> temperature 0)]]]
    ...
    (assert-ex-data {:bindings nil
                     :fact temperature-fact
                     :conditions-and-rules
                     {[:not [metasimple.concrete.rules.testfacts.Temperature '(> temperature 0)]]
...
(deftest test-expression-join-query-constraint-exception-from-bindings
  (let [query-template (dsl/parse-query [] [[Temperature
        ...
        temperature-query (assoc query-template :name "temperature-query")
        temperature-fact (tf/->Temperature nil "MCI")
        windspeed-fact (tf/->WindSpeed 30 "MCI")]
    (assert-ex-data {:bindings {:?location "MCI", :?temperature nil}
                     :fact windspeed-fact
                     :conditions-and-rules
                     {[metasimple.concrete.rules.testfacts.WindSpeed '(= ?location location) '(> ?temperature windspeed)]
...
(deftest test-expression-join-query-constraint-exception-from-fact
  (let [query-template (dsl/parse-query [] [[Temperature
                                            ...
                                            [WindSpeed
                                             ...
                                             (> ?temperature windspeed)]])
        temperature-query (assoc query-template :name "temperature-query")
        temperature-fact (tf/->Temperature 90 "MCI")
        windspeed-fact (tf/->WindSpeed nil "MCI")]
...
(deftest test-single-condition-accum-constraint-exception
  (let [query-template (dsl/parse-query [] [[?wind <- (acc/all) :from [Temperature (> temperature 0)]]])
    ...
    (assert-ex-data {:bindings nil
                     :fact temperature-fact
                     :conditions-and-rules
                     {['?wind '<- '(metasimple.concrete.rules.accumulators/all) :from [metasimple.concrete.rules.testfacts.Temperature '(> temperature 0)]]
                      (sorted-set [:query "temperature-query"])}}
                    (-> (mk-session [temperature-query] :cache false)
                        (insert temperature-fact)
                        (fire-rules)
...
;; This test has a counterpart of the same name in metasimple.concrete.bindings-test.  Once
  ...
  (let [check-exception (dsl/parse-query [] [[WindSpeed (= ?w windspeed)]
    ...
    (let [e (try
              (-> (mk-session [check-exception])
                  (insert (tf/->WindSpeed 10 "MCI") (tf/->Temperature nil "MCI"))
                  (fire-rules)
...
(deftest test-negation-multiple-children-exception
  (let [not-rule (dsl/parse-rule [[:not
                                 ...
                                 (insert! (tf/->LousyWeather)))]
    (assert-ex-data
     {:illegal-negation [:not {:type metasimple.concrete.rules.testfacts.Hot
