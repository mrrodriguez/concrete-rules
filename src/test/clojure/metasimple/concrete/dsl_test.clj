(ns metasimple.concrete.dsl-test
  (:require
...
(def-rules-test test-variable-visibility
  ...
  {:rules [temps-for-locations [[[:location (= ?loc (:loc this))]
                                 ...
                                 [Temperature
                                  (= ?temp temperature)
                                  ;; TODO: This comment was copied from metasimple.concrete.rules-test, is it still accurate?
                                  ;;
                                  ;; This can only have one binding right
                                  ;; now due to work that needs to be done
                                  ;; still in metasimple.concrete.rules.compiler/extract-from-constraint
