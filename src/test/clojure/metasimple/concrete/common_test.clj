(ns metasimple.concrete.common-test
  "Common tests for Concrete Rules in Clojure and ClojureScript."
  (:require
    [clojure.test :refer :all]
    [metasimple.concrete.rules :refer [defrule defsession defquery
...
(defsession params-with-filter-session [temps-below-threshold] :fact-type-fn :type)

(deftest test-accum-with-filter

  (is (= [{:?threshold 0, :?low-temps []}]
        (-> params-with-filter-session
  ...
  (let [results (-> params-with-filter-session
                  (insert {:type :temperature :value 20})
