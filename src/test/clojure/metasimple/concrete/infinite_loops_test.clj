(ns metasimple.concrete.infinite-loops-test
  (:require 
    ...
    [metasimple.concrete.rules.testfacts :refer [->Cold  ->Hot ->First ->Second]]
    [metasimple.concrete.tools.loop-detector :as ld]
    [metasimple.concrete.tools.testing-utils :refer [def-rules-test
                                                     ex-data-maps
                                                     side-effect-holder-fixture
                                                     side-effect-holder
                                                     assert-ex-data]]
    [metasimple.concrete.tools.tracing :as tr])
  (:import
...
(def-rules-test test-truth-maintenance-loop
  ...
  (let [watched-session (ld/with-loop-detection empty-session 3000 :throw-exception)]
    ...
    (assert-ex-data  {:concrete-rules/infinite-loop-suspected true}  (fire-rules watched-session))))
...
(def-rules-test test-truth-maintenance-loop-with-salience
  ...
  (doseq [session (map #(ld/with-loop-detection % 3000 :throw-exception)
    ...
    (assert-ex-data  {:concrete-rules/infinite-loop-suspected true}  (fire-rules session))))
...
(def-rules-test test-recursive-insertion
  ...
  (let [watched-session (ld/with-loop-detection empty-session 10 :throw-exception)]
    ...
    (assert-ex-data {:concrete-rules/infinite-loop-suspected true}
      (-> watched-session
        (insert (->Cold "ARN"))
    ...
    (try
      ...
      (catch Exception e
        (let [data-maps (ex-data-maps e)
              loop-data (filter :concrete-rules/infinite-loop-suspected data-maps)]
          (is (= (count loop-data)
                1)
...
(def-rules-test test-invalid-on-limit-fn-error
  ...
  (assert-ex-data
    {:concrete-rules/max-cycles-exceeded-fn "NOT A FUNCTION"}
