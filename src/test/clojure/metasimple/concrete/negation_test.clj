(ns metasimple.concrete.negation-test
  (:require
...
(def-rules-test test-complex-negation
  ...
  {:queries [cold-not-match-temp [[]
             ...
             ;; should not be exposed to Concrete Rules's internal facts, such as complex negation result markers, through query results.
  ...
  (let [no-system-types? (fn [session facts-expected?]
                           ...
                           (not-any? (fn [fact] (instance? metasimple.concrete.rules.engine.ISystemFact fact))
                             (as-> session x
