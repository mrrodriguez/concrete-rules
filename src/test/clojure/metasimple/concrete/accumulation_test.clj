(ns metasimple.concrete.accumulation-test
  (:require
    ...
    [metasimple.concrete.rules.testfacts :refer [->Temperature ->Cold ->WindSpeed
                                                 ->ColdAndWindy ->TemperatureHistory
...
;; The distinction between tests here and tests in files focused on the aspects of Concrete Rules that the DSL represents
...
;; Concrete Rules's value-based semantics won't consider the metadata part of the fact for the purpose of memory operations.
(defn type-or-class
  [fact]
  (or (:type fact)
    (type fact)))
