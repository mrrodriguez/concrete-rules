(ns metasimple.concrete.tools.inspect-test
  (:require
    [clojure.test :refer [is deftest run-tests testing use-fixtures]]
    [clojure.walk :as w]
    [metasimple.concrete.rules :refer [insert fire-rules insert! insert-unconditional! retract query]]
    ...
    [metasimple.concrete.tools.inspect :refer [inspect map->Explanation
                                               explain-activations
                                               with-full-logging
...
;; https://github.com/cerner/clara-rules/blob/0.15.2/src/main/clojure/metasimple/concrete/rules/schema.clj#L28
...
(tu/def-rules-test test-node-fn-name->production-name
  ...
  (testing "Happy Path"
    (let [prod-name-using-str (first (node-fn-name->production-name base-session
                                       ...
                                       "metasimple.concrete.tools.inspect-test/AN_1_AE"))
  ...
  (testing "unrecognized input"
    (try
      (node-fn-name->production-name base-session
        ...
        {:name "metasimple.concrete.tools.inspect-test/AN_1_AE"})
  ...
  (testing "Invalid function name format"
    (try
      (node-fn-name->production-name base-session
        "metasimple.concrete.tools.inspect-test/NOTAREALNODE_1_AE")
      (is false "Exception should be thrown")
      (catch ExceptionInfo exc
        (is (= (ex-data exc)
              {:name "metasimple.concrete.tools.inspect-test/NOTAREALNODE_1_AE"
               :simple-name "NOTAREALNODE-1-AE"})))))

  (testing "unable to find node in rulebase"
    (try
      (node-fn-name->production-name base-session
        "metasimple.concrete.tools.inspect-test/AN_9001_AE")
      (is false "Exception should be thrown")
      (catch ExceptionInfo exc
        (is (= (ex-data exc)
              {:node-id "9001"
