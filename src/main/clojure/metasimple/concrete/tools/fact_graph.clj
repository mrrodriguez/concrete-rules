(ns metasimple.concrete.tools.fact-graph
  (:require
    [metasimple.concrete.tools.inspect :as i]
    [schema.core :as sc]))
...
(defn session->fact-graph
  ...
  (let [id-counter (atom 0)
        ...
        ;; This function generates one of the entries in the map returned by metasimple.concrete.tools.inspect/inspect.
        ;; The function is private since it would be confusing for only one of the entries in metasimple.concrete.tools.inspect/inspect
        ...
        ;; Produce tuples of the form [inserted-fact {:rule rule :explanation metasimple.concrete.tools.inspect.Explanation}]
