(ns metasimple.concrete.tools.loop-detector
  (:require
    [metasimple.concrete.rules.engine :as eng]
    [metasimple.concrete.rules.listener :as l]
    [metasimple.concrete.tools.tracing :as trace]))
...
(defn throw-exception-on-max-cycles
  ...
  (let [trace (trace/listener->trace (l/to-persistent! (:listener eng/*current-session*)))]
    (throw (ex-info "Reached maximum activation group transitions threshhold; an infinite loop is suspected"
             (cond-> {:concrete-rules/infinite-loop-suspected true}
...
(defn on-limit-fn-lookup
  ...
  (cond
    ...
    :else (throw (ex-info "The :on-error-fn must be a non-nil function value" {:concrete-rules/max-cycles-exceeded-fn fn-or-keyword}))))
