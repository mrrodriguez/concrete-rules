(ns metasimple.concrete.tools.inspect
  ...
  (:require
    ...
    [metasimple.concrete.tools.internal.inspect :as i]
    [schema.core :as s])
  (:import
...
(s/defschema ConditionMatch
  "A structure associating a condition with the facts that matched them. The fields are: :fact - A
  fact propagated from this condition in a rule or query. For non-accumulator conditions, this will
  be the fact matched by the condition. For accumulator conditions, it will be the result of the
  accumulation. So, for example, if we have a condition like

         [?cold <- Cold]

         a ConditionMatch for this condition will have a Cold fact in its :fact field. If we have a
         condition like
  ...
  :condition - A structure representing this condition. This is the same structure used inside the
               structures defining rules and queries.

  :facts-accumulated (nullable) : When the condition is an accumulator condition, this will contain
                                  the individual facts over which the accumulator ran. For example,
                                  in the case above with the condition

                                  [?min-cold <- (acc/min :temperature) :from [Cold]]
                                  
                                  this will contain the individual Cold facts over which we
                                  accumulated, while the :fact field will contain the result of the
                                  accumulation."
  {:fact s/Any
...
(def ^{:doc "Return a new session without information gathering on this session for optional inspection keys.
  ...
  i/without-activation-listening)
...
(s/defn inspect
   ...
   * :unfiltered-rule-matches: A map of rule structures to their matching explanations.
     ...
     the caller are internal implementation details of Concrete Rules unless explicitly controlled by the user.
...
(defn explain-activations
  ...
  (metasimple.concrete.tools.inspect/explain-activations session
