(ns metasimple.concrete.rules.accumulators
...
(defn accum
   ...
   * An optional convert-return-fn that converts the reduced data into something useful to the caller.
    ...
    "
  [{:keys [_initial-value _reduce-fn _combine-fn _retract-fn _convert-return-fn] :as accum-map}]
...
(defn reduce-to-accum
  "Creates an accumulator using a given reduce function with optional initial value and
   ...
   (metasimple.concrete.rules.accumulators/max :temperature :returns-fact true),
