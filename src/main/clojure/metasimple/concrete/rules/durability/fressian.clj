(ns metasimple.concrete.rules.durability.fressian
  ...
  (:require
    ...
    [metasimple.concrete.rules.durability :as d]
    [metasimple.concrete.rules.engine :as eng]
    [metasimple.concrete.rules.memory :as mem]
    [metasimple.concrete.rules.platform :as pform]
    [schema.core :as s])
  (:import
    (java.io
      InputStream
...
(def ^:private ^Map class->factory-fn-sym
...
(def handlers
  ...
  {"java/class"
   ...
   "metasimple/concrete/productionnode"
   (create-cached-node-handler ProductionNode
     "metasimple/concrete/productionnode"
     "metasimple/concrete/productionnodeid"
     #(assoc % :rhs nil)
     d/add-rhs-fn)

   "metasimple/concrete/querynode"
   (create-cached-node-handler QueryNode
     "metasimple/concrete/querynode"
     "metasimple/concrete/querynodeid")

   "metasimple/concrete/alphanode"
   (create-cached-node-handler AlphaNode
     "metasimple/concrete/alphanodeid"
     "metasimple/concrete/alphanode"
     #(assoc % :activation nil)
     d/add-alpha-fn)

   "metasimple/concrete/rootjoinnode"
   (create-cached-node-handler RootJoinNode
     "metasimple/concrete/rootjoinnode"
     "metasimple/concrete/rootjoinnodeid")

   "metasimple/concrete/hashjoinnode"
   (create-cached-node-handler HashJoinNode
     "metasimple/concrete/hashjoinnode"
     "metasimple/concrete/hashjoinnodeid")

   "metasimple/concrete/exprjoinnode"
   (create-cached-node-handler ExpressionJoinNode
     "metasimple/concrete/exprjoinnode"
     "metasimple/concrete/exprjoinnodeid"
     #(assoc % :join-filter-fn nil)
     d/add-join-filter-fn)

   "metasimple/concrete/negationnode"
   (create-cached-node-handler NegationNode
     "metasimple/concrete/negationnode"
     "metasimple/concrete/negationnodeid")

   "metasimple/concrete/negationwjoinnode"
   (create-cached-node-handler NegationWithJoinFilterNode
     "metasimple/concrete/negationwjoinnode"
     "metasimple/concrete/negationwjoinnodeid"
     #(assoc % :join-filter-fn nil)
     d/add-join-filter-fn)

   "metasimple/concrete/testnode"
   (create-cached-node-handler TestNode
     "metasimple/concrete/testnode"
     "metasimple/concrete/testnodeid"
     #(assoc % :test nil)
     d/add-test-fn)

   "metasimple/concrete/accumnode"
   (create-cached-node-handler AccumulateNode
     "metasimple/concrete/accumnode"
     "metasimple/concrete/accumnodeid"
     #(assoc % :accumulator nil)
     d/add-accumulator)

   "metasimple/concrete/accumwjoinnode"
   (create-cached-node-handler AccumulateWithJoinFilterNode
     "metasimple/concrete/accumwjoinnode"
     "metasimple/concrete/accumwjoinnodeid"
     #(assoc % :accumulator nil :join-filter-fn nil)
     (comp d/add-accumulator d/add-join-filter-fn))

   "metasimple/concrete/ruleorderactivation"
   {:class RuleOrderedActivation
    :writer (reify WriteHandler
              (write
                [_ w c]
                (.writeTag w "metasimple/concrete/ruleorderactivation" 4)
    ...
    :readers {"metasimple/concrete/ruleorderactivation"
              (reify ReadHandler
                (read
                  [_ rdr tag component-count]
   ...
   "metasimple/concrete/memidx"
   {:class MemIdx
    :writer (reify WriteHandler
              (write
                [_ w c]
                (.writeTag w "metasimple/concrete/memidx" 1)
                (.writeInt w (:idx c))))
    :readers {"metasimple/concrete/memidx"
              (reify ReadHandler
                (read
                  [_ rdr tag component-count]
                  (d/find-mem-idx (.readObject rdr))))}}

   "metasimple/concrete/internalmemidx"
   {:class InternalMemIdx
    :writer (reify WriteHandler
              (write
                [_ w c]
                (.writeTag w "metasimple/concrete/internalmemidx" 1)
                (.writeInt w (:idx c))))
    :readers {"metasimple/concrete/internalmemidx"
              (reify ReadHandler
                (read
                  [_ rdr tag component-count]
...
(defrecord FressianSessionSerializer
