(ns metasimple.concrete.rules.durability
  ...
  (let [cname (-> s meta ::comparator-name)]
    ...
    (when (and (not cname)
      ...
      (throw (ex-info (str "Cannot serialize sorted collection with non-default"
                        " comparator because no :metasimple.concrete.rules.durability/comparator-name provided in metadata.")
               {:sorted-coll s
...
;; Similar what is in metasimple.concrete.rules.memory currently, but just copied for now to avoid dependency issues.
(defn- update-vals
  [m update-fn]
  (->> m
    (reduce-kv (fn [m k v]
...
(defn- index-update-bindings-keys
...
(defn index-alpha-memory
  [internal-seen seen amem]
...
(defn index-beta-memory
  [internal-seen seen bmem]
...
(defn index-production-memory
  [internal-seen seen pmem]
...
(defn index-activation-map
  [internal-seen seen actmap]
  (update-vals actmap
...
(defn indexed-session-memory-state
   ...
   * :indexed-facts - the facts replaced with MemIdx placeholders.  The facts are returned in a
   ...
   Note!  This function should not typically be used.  It is left public to assist in ISessionSerializer
          durability implementations.  Use metasimple.concrete.rules/mk-session typically to make rule sessions.
  ...
  (let [idx-fact-arr-pair-fn (fn [^java.util.Map$Entry e]
        ...
        vec-indexed-facts (fn [^Map fact->index-map
                            ...
                            ;; JDK6 support is still expected to work for Concrete Rules.  The only trasducer
...
(defn assemble-restored-session
  "Builds a Concrete Rules session from the given rulebase and memory components.  When no memory is given a new 
   one is created with all of the defaults of eng/local-memory.
   Note!  This function should not typically be used.  It is left public to assist in ISessionSerializer 
          durability implementations.  Use metasimple.concrete.rules/mk-session typically to make rule sessions.

   If the options are not provided, they will default to the Concrete Rules session defaults.  The available options
   ...
   (let [{:keys [listeners transport]} opts]
     ...
     (eng/assemble {:rulebase rulebase
                    :memory (eng/local-memory rulebase
                              (metasimple.concrete.rules.engine.LocalTransport.)
                    ...
                    :transport (or transport (metasimple.concrete.rules.engine.LocalTransport.))
...
(defn rulebase->rulebase-with-opts
  "Intended for use in rulebase deserialization implementations where these functions were stripped
   ...
   as metasimple.concrete.rules/mk-session.  This function should typically only be used when implementing ISessionSerializer."
...
(defprotocol ISessionSerializer
   ...
   * :forms-per-eval - The maximum number of expressions that will be evaluated per call to eval.
     ...
     Defaults to 5000, see metasimple.concrete.rules.compiler/forms-per-eval-default for more information.

   Options for the rulebase semantics that are documented at metasimple.concrete.rules/mk-session include:
...
(defprotocol IWorkingMemorySerializer
  "Provides the ability to serialize and deserialize the facts stored in the working memory of a session.
   ...
   in memory can vary greatly as a result of this.  Concrete Rules does not yet provide any default implementations
   for this, but may in the future.  However, many of the handlers defined in metasimple.concrete.rules.durability.fressian
   can be reused if the consumer wishes to serialize via Fressian.  See more on this in 
   the metasimple.concrete.rules.durability.fressian namespace docs.
...
(s/defn serialize-session-state
  "Serializes the state of the given session.  By default, this *excludes* the rulebase from being
   ...
   metasimple.concrete.rules.durability.fressian/create-session-serializer.  
   See metasimple.concrete.rules.durability.fressian for more specific details regarding this, including the extra
...
(s/defn deserialize-session-state
  :- (s/protocol eng/ISession)
