(ns metasimple.concrete.rules.memory
...
(defn fast-token-compare
  [compare-fn token other-token]
   ...
   (let [node (:node activation)]
     (RuleOrderedActivation. (:id node)
       ...
       (or (-> node
             :production
             meta
             :metasimple.concrete.rules.compiler/rule-load-order)
         0)
       use-token-identity?))))
...
(deftype TransientLocalMemory
  [rulebase
   ...
   ^:unsynchronized-mutable ^java.util.NavigableMap activation-map]
  ...
  (add-elements!
    [_memory node join-bindings elements]
  ...
  (add-tokens!
    [_memory node join-bindings tokens]
  ...
  (add-accum-reduced!
    [_memory node join-bindings accum-result fact-bindings]
...
(defrecord PersistentLocalMemory
  [rulebase
   activation-group-sort-fn
   activation-group-fn
   alphas-fn
