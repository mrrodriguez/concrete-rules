(ns metasimple.concrete.rules.engine
  ...
  (:require
    ...
    [metasimple.concrete.rules.listener :as l]
    [metasimple.concrete.rules.memory :as mem]
    [metasimple.concrete.rules.platform :as platform]
    [metasimple.concrete.rules.update-cache.cancelling :as ca]
    [metasimple.concrete.rules.update-cache.core :as uc]
    [schema.core :as s]))
...
(defrecord Accumulator
  [initial-value retract-fn reduce-fn combine-fn convert-return-fn])
...
(defprotocol ISession
  ...
  ;;
  ;; Note that metasimple.concrete.rules/fire-rules, the public API for these methods, will handle
...
(defn- propagate-items-to-nodes
  [transport memory listener nodes items propagate-fn]
  (doseq [node nodes
...
(defrecord ProductionNode
  [id production rhs]

  ILeftActivate

  (left-activate
    [node join-bindings tokens memory transport listener]
...
(defrecord AlphaNode
  [id env children activation fact-type]

  IAlphaActivate

  (alpha-activate
    [node facts memory transport listener]
...
(defrecord RootJoinNode
  [id condition children binding-keys]
  ...
  (right-activate
    [node join-bindings elements memory transport listener]
...
(defrecord ExpressionJoinNode
  [id condition join-filter-fn children binding-keys]
...
;; on the alpha side.
(defrecord NegationWithJoinFilterNode
  [id condition join-filter-fn children binding-keys]
...
(defrecord TestNode
  [id env constraints test children]
...
(defrecord AccumulateNode
  [id accum-condition accumulator result-binding children binding-keys new-bindings]
  ...
  (right-activate-reduced
    [node join-bindings fact-seq memory transport listener]
...
(defrecord AccumulateWithJoinFilterNode
  [id accum-condition accumulator join-filter-fn
  ...
  (right-activate-reduced
    [node join-bindings binding-candidates-seq memory transport listener]
...
(deftype LocalSession
  ...
  (insert
    ...
    (let [new-pending-operations (conj pending-operations (uc/->PendingUpdate :insertion
                                                            ...
                                                            ;; collection of facts after passing it to Concrete Rules for the constituent
  ...
  ;; Prior to issue 249 we only had a one-argument fire-rules method.  metasimple.concrete.rules/fire-rules will always call the two-argument method now
...
(defn assemble
  "Assembles a session from the given components, which must be a map
   ...
   :rulebase A recorec matching the metasimple.concrete.rules.compiler/Rulebase structure.
   :memory An implementation of the metasimple.concrete.rules.memory/IMemoryReader protocol
   :transport An implementation of the metasimple.concrete.rules.engine/ITransport protocol
   :listeners A vector of listeners implementing the metasimple.concrete.rules.listener/IPersistentListener protocol
...
(defn options->activation-group-sort-fn
  ...
  only be called by Concrete Rules itself."
  [options]
...
(def ^:private internal-salience-levels
  {:default 0
  ...
  (let [rule-salience-fn (or (:activation-group-fn options)
    ...
    (fn [production]
      [(rule-salience-fn production)
       (internal-salience-levels (or (some-> production :props :concrete-rules/internal-salience)
                                   :default))])))
