(ns metasimple.concrete.rules.compiler
  ...
  (:require
   ...
   [metasimple.concrete.rules.schema :as schema]
   [schema.core :as sc])
  (:import
   (clojure.lang
    IFn)
...
(sc/defrecord Rulebase
              [;; Map of matched type to the alpha nodes that handle them.
...
(defn- equality-expression?
  ...
  (let [qualify-when-sym #(when-let [resolved (and (symbol? %)
        ...
        op (first expression)]
    ;; Check for unqualified = or == to support original Concrete Rules unification
...
(defn- mk-node-fn-name
  "A simple helper function to maintain a consistent pattern for naming anonymous functions in the rulebase.
   ...
   node-type - expected to align with one of the types of nodes defined in metasimple.concrete.rules.engine, and node-type->abbreviated-type.
...
(defn compile-test-handler
  [node-id constraints env]
...
(defn compile-action
  ...
  (let [;; Avoid creating let bindings in the compile code that aren't actually used in the body.
        ...
        ;; providing it to Concrete Rules.
...
(defn compile-join-filter
  ...
  [node-id node-type {:keys [type constraints args]} ancestor-bindings element-bindings env]
  ...
  (for [condition conditions
        expanded (if (= :exists (condition-type condition))
                   ...
                   (let [exists-count (gensym "?__gen__")]
                     [{:accumulator '(metasimple.concrete.rules.accumulators/exists)
...
(sc/defn analyze-condition
  :- {;; Variables used in the condition that are bound
      :bound #{sc/Symbol}
...
(sc/defn condition-to-node
  :- schema/ConditionNode
...
(sc/defn ^:private add-node
  ...
  [beta-graph :- schema/BetaGraph
   ...
   target-node :- (sc/conditional
                   #(#{:production :query} (:node-type %)) schema/ProductionNode
...
(sc/defn ^:private extract-negation
  ...
  (if (and (= :not (first expression))
    ...
    (let [negation-expr (second expression)
          ...
          modified-expression `[:not {:type ~(if (compiling-cljs?)
                                               'metasimple.concrete.rules.engine/NegationResult
                                               'metasimple.concrete.rules.engine.NegationResult)
          ...
          generated-rule (cond-> {:name gen-rule-name
                                  ...
                                  :rhs `(metasimple.concrete.rules/insert! (eng/->NegationResult ~gen-rule-name
                           ...
                           true (assoc-in [:props :concrete-rules/internal-salience] :extracted-negation)
...
(def ^:private empty-beta-graph
  {:forward-edges (sorted-map)
...
(sc/defn ^:private add-conjunctions
  :- {:beta-graph schema/BetaGraph
      :new-ids [sc/Int]
      :bindings #{sc/Keyword}}
...
(sc/defn ^:private add-production
  :- schema/BetaGraph
...
(sc/defn extract-exprs
  :- schema/NodeExprLookup
...
(sc/defn ^:private compile-beta-graph
  :- {sc/Int sc/Any}
...
(deftype AlphaRootsWrapper
         [fact-type ^int fact-type-hash roots]
...
(sc/defn build-network
  ...
  (let [beta-nodes (vals id-to-node)
        ...
        alpha-nodes (for [{:keys [id type alpha-fn children env] :as _alpha-map} alpha-fns
...
(defn clear-session-cache!
  "Clears the cache of reusable Concrete Rules sessions, so any subsequent sessions
...
(defn production-load-order-comp
  [a b]
...
(sc/defn mk-session*
  ...
  (let [;; We need to put the productions in a sorted set here, instead of in mk-session, since we don't want
        ...
        productions (with-meta (into (sorted-set-by production-load-order-comp)
                      ...
                      {:metasimple.concrete.rules.durability/comparator-name `production-load-order-comp})
