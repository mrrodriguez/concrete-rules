...
(defn insert-unconditional!
  "To be executed within a rule's right-hand side, this inserts a new fact or facts into working memory.
   ...
   Concrete Rules's truth maintenance."
  [& facts]
...
(defn retract!
  ...
  Concrete Rules's underlying truth maintenance retract inserted items if their support becomes false."
  [& facts]
  (eng/rhs-retract-facts! facts))

(defn accumulate
  "DEPRECATED. Use metasimple.concrete.rules.accumulators/accum instead.
...
(defmacro mk-session
  "Creates a new session using the given rule sources. The resulting session
      ...
      * :ancestors-fn, which returns a collection of ancestors for a given type. Defaults to Clojure's ancestors function. A
        ...
        this ordering will be maintained by Concrete Rules; ordering the ancestors consistently will increase the consistency of overall performance.
      ...
      * :omit-compile-ctx - When false Concrete Rules, in Clojure, retains additional information to improve error messages during
        ...
        (ie. via Concrete Rules's durability support).
        Defaults to true, see metasimple.concrete.rules.compiler/omit-compile-ctx-default for more information.
...
(defmacro defrule
  ...
  See the [rule authoring documentation](http://www.concrete-rules.org/docs/rules/) for details."
  [name & body]
...
(defmacro defquery
  ...
  See the [query authoring documentation](http://www.concrete-rules.org/docs/queries/) for details."
  [name & body]
  (let [doc (if (string? (first body)) (first body) nil)]
